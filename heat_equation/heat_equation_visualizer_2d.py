...
model.load_state_dict(torch.load("heat_equation_2d.pt", map_location=device))
model.eval()
...
with torch.no_grad():
    ...
    initial_pred = model(X_flat, Y_flat, initial_t_flat, alpha_flat).reshape(100, 100).cpu().numpy()
    initial_exact = heat_calculation(X, Y, initial_t, alpha).cpu().numpy()
...
def update(frame_index):
    ...
    with torch.no_grad():
        U_pred = model(X_flat, Y_flat, t_flat, alpha_flat).reshape(100, 100).cpu()
        U_exact = heat_calculation(X, Y, t_scalar, alpha).cpu()
...
ani.save('heat_equation_2d_visualization.mp4', writer=writer)
